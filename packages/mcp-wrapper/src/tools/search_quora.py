"""``search_quora`` MCP tool implementation.

Searches Quora Q&A via a SerpApi Google search filtered to quora.com.
Returns a normalised list of organic results.
...
3. Check rate limit.
4. Call SerpApi (Google engine with site:quora.com filter).
5. Normalise ``organic_results``.
6. Store in cache.
...
from src.middleware.validator import validate_search_quora_inputs
from src.serpapi.client import SerpApiClient
from src.serpapi.normalizer import normalize_organic_results

logger = logging.getLogger(__name__)


async def execute_search_quora(
    query: str,
    num_results: int = 10,
    *,
    ...
    """Search Quora for Q&A content matching a query.
    ...
    # 1. Validate inputs.
    errors = validate_search_quora_inputs(query, num_results)
    if errors:
    ...
    if cached_value is not None:
        logger.info("Cache HIT — search_quora query=%r", query)
    ...
    # 4. Call SerpApi using Google engine with site filter.
    filtered_query = f"site:quora.com {query}"
    try:
        raw_response = await client.search_web(
    ...
    # 5. Normalise.
    try:
        normalised = normalize_organic_results(
            raw_response,
            query=query,
            cached=False,
    ...
    logger.info(
        "search_quora OK — query=%r results=%d request_id=%s",
        query,
