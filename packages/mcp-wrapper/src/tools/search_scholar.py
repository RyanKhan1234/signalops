"""``search_scholar`` MCP tool implementation.

Performs a Google Scholar search via SerpApi and returns a normalised list
of academic organic results.  Follows the same middleware pipeline as all
other MCP tools.
...
3. Check rate limit.
4. Call SerpApi (google_scholar engine).
...
from src.middleware.validator import validate_search_scholar_inputs
...
async def execute_search_scholar(
    query: str,
    num_results: int = 10,
    *,
    ...
    """Search Google Scholar for academic papers and articles.
    ...
    # 1. Validate inputs.
    errors = validate_search_scholar_inputs(query, num_results)
    if errors:
    ...
    if cached_value is not None:
        logger.info("Cache HIT — search_scholar query=%r", query)
    ...
    # 4. Call SerpApi.
    try:
        raw_response = await client.search_scholar(
            query=query,
    ...
    logger.info(
        "search_scholar OK — query=%r results=%d request_id=%s",
        query,
