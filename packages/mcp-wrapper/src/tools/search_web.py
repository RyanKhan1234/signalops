"""``search_web`` MCP tool implementation.

Performs a general Google web search via SerpApi and returns a normalised
list of organic results.  Follows the same middleware pipeline as all other
MCP tools.
...
from src.middleware.validator import validate_search_web_inputs
...
async def execute_search_web(
    ...
    *,
    ...
    # 1. Validate inputs.
    errors = validate_search_web_inputs(query, num_results)
    if errors:
    ...
    if cached_value is not None:
        logger.info("Cache HIT — search_web query=%r", query)
    ...
    logger.info(
        "search_web OK — query=%r results=%d request_id=%s",
        query,
