...
This module contains the core business logic for the ``search_news`` tool.
The MCP server calls ``execute_search_news`` directly; the function is
intentionally decoupled from the MCP framework so it can be unit-tested
independently.
...
from src.middleware.validator import validate_search_news_inputs
...
async def execute_search_news(
    ...
    *,
    ...
    # 1. Validate inputs.
    errors = validate_search_news_inputs(query, time_range, num_results)
    if errors:
    ...
    cache_key = ResponseCache.make_key(
        "search_news",
        {"query": query, "time_range": time_range, "num_results": num_results},
    )
    cached_value = cache.get(cache_key)
    if cached_value is not None:
        logger.info("Cache HIT — search_news query=%r", query)
    ...
    try:
        ...
        # Wrap other httpx errors (connect errors, etc.) as upstream errors
    ...
    logger.info(
        "search_news OK — query=%r results=%d request_id=%s",
        query,
